...
export const Route = createFileRoute("/vault")({
  head: () => ({
    meta: [{ title: "Document Vault — GateFlow" }],
  }),
  component: VaultPage,
});

const placeholderDocs = [
  { id: "1", name: "Passport — Chestnut King.pdf", category: "Identity", size: "1.2 MB", date: "2026-03-12", horse: "Chestnut King" },
  { id: "2", name: "Health Certificate — Annual 2026.pdf", category: "Health", size: "840 KB", date: "2026-01-08", horse: "All" },
  { id: "3", name: "Sale Contract — Firefly.docx", category: "Legal", size: "220 KB", date: "2025-12-01", horse: "Firefly" },
  { id: "4", name: "Embryo Transfer Agreement.pdf", category: "Breeding", size: "1.8 MB", date: "2026-02-14", horse: "N/A" },
  { id: "5", name: "Insurance Policy 2026.pdf", category: "Finance", size: "3.1 MB", date: "2026-01-01", horse: "All" },
  { id: "6", name: "Training Log — Q1 2026.xlsx", category: "Training", size: "590 KB", date: "2026-04-01", horse: "Multiple" },
];
...
function VaultPage() {
  ...
  const filtered = placeholderDocs.filter((d) => {
    const matchSearch = d.name.toLowerCase().includes(search.toLowerCase()) || d.horse.toLowerCase().includes(search.toLowerCase());
  ...
  return (
    <AppShell>
      <div className="flex items-baseline justify-between mb-8">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            {placeholderDocs.length} documents · Passports, contracts, health certificates &amp; more
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filtered.map((doc) => (
          <div key={doc.id} className="lux-card p-5 group hover:border-primary/30 transition-colors cursor-pointer">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-muted-foreground mt-1">{doc.horse} · {doc.size}</div>
                <div className="text-xs text-muted-foreground">{doc.date}</div>
              </div>
            </div>
            <div className="mt-3 flex items-center justify-between">
              <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${catColors[doc.category] ?? "bg-secondary"}`}>
                {doc.category}
              </span>
