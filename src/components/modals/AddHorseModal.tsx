...
export function AddHorseModal({ open, onOpenChange }: Props) {
  ...
  const stallions = useMemo(() => horses.filter((h) => h.sex === "Reproductor"), [horses]);
  const mares = useMemo(() => horses.filter((h) => h.sex === "Yegua"), [horses]);
  ...
  const reset = () => {
    ...
    setRegisteredName("");
    setGaitType("Paso Fino Colombiano");
    setMovementCategory("");
    setTrainingLevel("Proceso");
    setMorphologyNotes("");
    setCriadero("");
    setBreederName("");
    setRegistrationCategory("Reporte de Monta");
    setFedequinasId("");
    setAssociation("");
    setBrio("5");
    setNobleza("5");
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setError(err?.message || (typeof err === "string" ? err : "Could not create the horse."));
    }
  };
