...
export function EditHorseModal({ open, onOpenChange, horse }: Props) {
  ...
  const stallions = useMemo(() => horses.filter((h) => h.sex === "Stallion" && h.id !== horse.id), [horses, horse.id]);
  const mares = useMemo(() => horses.filter((h) => h.sex === "Mare" && h.id !== horse.id), [horses, horse.id]);
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Could not update the horse.");
    }
  };
