...
export function EditHealthRecordModal({ open, onOpenChange, record }: Props) {
  ...
  // Pre-populate fields when record changes
  useEffect(() => {
    if (!record) return;
    setType((record.type as RecordType) ?? "vet_visit");
    setHorseId(record.horse_id ?? "");
    setTitle(record.title ?? "");
    setProfessional(record.professional ?? "");
    setDate(record.date ?? new Date().toISOString().slice(0, 10));
    setDiagnosis(record.diagnosis ?? "");
    setPrescription(record.prescription ?? "");
    setDose(record.dose ?? "");
    setFrequency(record.frequency ?? "");
    setProductUsed(record.product_used ?? "");
    setProductQuantity(record.product_quantity ?? "");
    setCost(record.cost ?? "");
    setRecurrence((record.recurrence as Recurrence) ?? "none");
    setStatus((record.status as Status) ?? "completed");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateRecord.mutateAsync({
        id: record.id,
        horse_id: horseId,
        horse_name: horse?.name ?? record.horse_name ?? "Unknown",
        type,
        title,
        notes: notes || "",
      ...
      console.error(err);
      toast.error(err?.message || "Could not update the health record.");
    }
  };

  return (
    <Modal open={open} onClose={handleClose} title="Edit health record" size="lg">
      {done ? (
        <div className="p-10 flex flex-col items-center gap-4">
          <span className="grid h-16 w-16 place-items-center rounded-full bg-primary/10 text-primary">
            <Check className="h-8 w-8" />
          </span>
          <p className="font-display text-2xl">Record updated</p>
          <p className="text-muted-foreground text-sm">Health record has been updated successfully.</p>
        </div>
      ) : (
