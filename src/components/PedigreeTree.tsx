...
export function PedigreeTree({ horse, allHorses = [], bloodline, onEditClick }: Props) {
  const parts = (bloodline || "Desconocido × Desconocido").split(/[×xX]/);
  const activeHorse = horse || ({
    ...
    sire_id: null,
    dam_id: null,
    sire_name: parts[0]?.trim() || "Desconocido",
    dam_name: parts[1]?.trim() || "Desconocido",
  } as unknown as Horse);
  ...
  const resolveHorse = (id: string | null, name: string | null) => {
    ...
    return { horse: null, name: name || "Desconocido", isInternal: false };
  };
  ...
  const paternalGrandSire = sireData.horse
    ...
    : { horse: null, name: "Desconocido", isInternal: false };
  ...
  const AncestorCard = ({
    ...
    const isUnknown = data.name === "Desconocido";
